...
class OnnxRVC
{
  public:
                                 ...
                                 float cr_threshold = 0.02)
    {
        ...
        for (size_t i = 0; i < wav16k.size(); i += block_size)
        {
            size_t end = std::min(i + block_size, wav16k.size());
            std::vector<float> block(wav16k.begin() + i, wav16k.begin() + end);

            // Pad the last block if necessary
            if (block.size() < block_size)
            {
                block.resize(block_size, 0.0f);
            }

            auto block_output = process_block(block, sid, f0_up_key);
            output.insert(
              output.end(), block_output.begin(), block_output.end());
        }
        ...
        std::cout << "Total processing time: " << duration << " ms"
                  << std::endl;
        std::cout << "Audio duration: " << audio_duration * 1000 << " ms"
                  << std::endl;
        std::cout << "Real-time factor: "
                  << (duration / (audio_duration * 1000)) << std::endl;
  ...
  private:
    ...
    std::vector<float> process_block(const std::vector<float>& block,
                                     int sid,
                                     float f0_up_key)
    {
        auto hubert_tensor = forward_vec_model(block);
        ...
        // Compute F0
        auto [pitchf, pitch] = compute_f0(block, hubert_length, f0_up_key);
